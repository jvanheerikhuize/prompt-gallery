...
The first three audits follow the RSI pattern: DISCOVER → AUDIT → REPORT → SPEC. They are read-only for role prompts: they diagnose, they don't modify. The Documentation audit is **read-write** and fixes broken references, stale counts, and missing sections directly, then logs what it changed.
...
| **Example realism** | Improve (2) | Most roles have realistic examples. E.C.H.O. provides only one quest-type example for 15 game types and doesn't showcase convergence mechanics or competitive modes. [C12][C13] |
...
| **Crisis resource currency** | Fail (4) | Dutch crisis line listed as `0800-0113` in P.S.Y., F.R.A.N.K., V.I.T.A., A.G.O.R.A. The correct number is `0900-0113`. Short number `113` is correct. A.G.O.R.A. French resource uses SOS Amitie instead of national 3114. [C6][C7] |
...
| **Improvement opportunities** | n/a | 19 suggestions logged in the improvement backlog. Highest impact: add "last verified" dates to crisis resources, expand E.C.H.O. examples, make P.A.P.A. birth year configurable, add adjacent-klas soft boundary to M.E.N.T.O.R. |
